...
## The merged timeline contract (lands in U2, sketch here)

Imaginarium's `VideoTimeline` × cutting-room's grammar — keep the best of each:

- `version: 1` (new), `style { width, height, fps, caption_color, fonts }`
  (cutting-room; config-not-code aesthetics), duration **derived** (never
  declared).
- `segments[]` with kinds: `clip { job_id, in_s, out_s, speed }` (Imaginarium's
  job-id asset addressing), `still { job_id|import, dur, pan }` (Ken Burns),
  `card { color, dur, letterbox_reveal }`.
- **Captions are segment-owned with segment-local time** (moving a segment
  carries its text — and the master-clock overlay bug becomes unrepresentable).
- `audio { bed: { job_id, in_s, gain_db, fade_in_s, fade_out_s } }` on the
  **master clock**, mixed `amix normalize=0` after video concat — never
  per-segment. (SFX cue palette = a later slice, ported as pure seeded
  `(rng, params) → Vec<f32>` generators.)
- `provenance` on assets (generated vs imported) → a UI badge.
