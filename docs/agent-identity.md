...
> token so the desktop/web/PWA client no longer needs the shared node secret (3e, server side).
> Remaining: the 3e **UI wiring** (IdentityWizard → login → WS-with-token, the companion slice);
> and the still-optional polish — a "set default + skip" persistence pass, binding-eviction on
