...
| `session_search` / `session_list` | verbatim retrieve over on-disk session JSONL (not Cerebro; default = this session; keyword AND) | specs `main.rs` `session_search_spec` / `session_list_spec`, intercept `supervisor.rs` | allow |
