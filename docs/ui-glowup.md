...
| **G6.1** | Temporal markers | André request: tiny date/time blocks at intervals in chat + a real taskbar tray clock. Gives the agent a sense of temporality (prevents stale-cerebro-recall confusion) AND traceability. Needs a Rust→Slint time source (a 1s/1min timer feeding a `Clock` global). |
