...
>
> Status: **shipped through G6.1** (plan locked 2026-06-11; G0–G6.1 done & live, G7 polish/tier pass remains).
> The agent-driven WM layer (`ui_*` verbs, `/state`, geometry persistence, reflexes) shipped after G6.1 — see `docs/adaptive-ui.md`.
...
*(plan-time list — the live catalog is the 21-AppKind `APP_TABLE` in ui-slint `main.rs`;
see `docs/adaptive-ui.md` §2 and `docs/app-parity.md`)*:
...
| **Terminal** | `/terminal-ws` PTY (backend done) | line-mode pane shipped (G3d); full VTE cursor grid remains deferred |
...
| **G5** ◑ | Persona behavior | **TIER-1 DONE** (verified live): surface-gating — Simple/mom hides the deep-tech apps (Sensors/Terminal/Council) from Start, collapses tool-call internals in chat (status + approve/reject kept; args/output un-expandable), reads at 1.15× chat font. New Settings → **PERSONA & UI STYLE** dropdown (collapsed since `#277` — current face + chevron, tiles expand on tap; was six always-visible tiles), live switch. Behaviour flags derive in the `Personas` global (`show-tech-apps` / `hide-tool-internals` / `font-scale`); the catalogue (`defs`) + switch (`pick`) also moved onto the global so the title-bar picker, wizard, and Settings pane share one source with zero prop-threading. **TIER-2 (agent style preamble) DONE**: the active persona reaches agentd over the WS (`set_persona` frame on switch/connect + a `persona` field on `hello{new}`), is stored per session (`apexos_core::PersonaSessions`, mirroring `SessionBindings`), and `root_turn` resolves it (explicit persona → else the bound agent's `default_skin`) to a response-style fragment (`persona_style`, pure + tested) that `compose_system` appends after soul/embodiment/priming — so the agent's *voice* matches the face (warm+plain for mom, terse+telemetry for the kid). Default/unknown persona → no fragment (common path byte-identical). Verified end-to-end with a mock provider: the mom turn's system prompt carried the fragment, the apex turn's did not. |
| **G6** ✓ | **Win-98 showcase** | full-fidelity chrome — **DONE** (verified live on André's laptop: "THE win-98 look on the dot"). New `components/win98.slint` = self-contained `Win98` token global (face/hilite/light/shadow/dark ramp + navy title + teal desktop) + `BevelEdges` (2-ring layered-1px-rect 3D bevel overlay, raised/sunken) + `Win98Button` (raised→sunken-on-press). Gated on `Personas.chrome == ChromeKind.win`. Retuned **Theme.Windows** palette from Aero→classic 98 grey/white/navy, radius 0 (only windows-dad uses it). Window chrome: navy-gradient caption + white bold title + 3 beveled caption buttons + raised frame bevel. Taskbar: flush grey beveled bar (34px, no float/round) + iconic Start button (4-pane flag) + sunken task tabs + sunken tray. Start menu: grey beveled panel + left navy banner w/ rotated "ApexOS" wordmark + navy-on-white hover rows. Flat teal desktop. **Deferred (resource items, not blockers):** MS-Sans libre font embed (§9), optional startup chime. (Tray clock shipped in G6.1 — verified live.) **Zero Rust changes** (pure Slint). |
| **G6.1** ✓ | Temporal markers | **DONE (both halves)** (tray clock verified live; chat divider built, live-pending agentd). André request: temporal grounding for traceability + to stop the agent treating stale cerebro recalls as current. New `Clock` global (in types.slint, exported) fed by a 1s `slint::Timer` in main.rs (chrono `Local`). Tray clock `HH:MM` in the Win-98 sunken tray + the modern bar (all personas). Chat **time-dividers**: `maybe_push_time_divider()` drops a centered `role="time"` MessageItem (label in `text`, ~3-min interval guard, reset on `clear_messages`) on the send + voice paths; `chat_view` renders it as a centered chip. chrono added to ui-slint (workspace dep). **✅ DONE (#102, since reworked):** the *model-facing* half — `build_ambient_clock()` (agentd main.rs: `[ambient — this node's live clock] Now: <YYYY-MM-DD HH:MM (Day)> UTC · uptime …`). Originally an embodiment-block line, but prompt caching shipped and a per-minute value inside the cached system prefix busts it — so `turn.rs::inject_ambient` now appends it **ephemerally to the latest user turn** (never persisted), gated per session (first turn + after an idle gap, `AGENTD_AMBIENT_GAP_SECS` default 600s). Mirrors the UI tray clock + dividers. |
...
- ⚠️ **Terminal app** = rendering a text grid + ANSI in Slint (no xterm.js); line-mode pane shipped (G3d), full VTE cursor grid → later phase.
...
|------|-------|--------------------|-------|
| **Feedback subsystem** (toasts + notifications, unified) | **Foundational** | core = small; center = medium | ✅ **DONE** — toast core (e32ae46, verified live) + notification center (G3c) |
| **Thermal pixel grid** (MLX90640 32×24 heatmap) | Feature / delight | ~~medium~~ | ✅ **DONE (#105)** — shipped per the corrected-scope design below |
| **Council / sub-agent visibility** | Surface | badge = small; app = medium | ✅ **DONE** — taskbar `⚖` badge + **Council app** (G3d) |
| **PTY terminal** | App | read-only = small; interactive = large | ✅ line-mode Terminal shipped (G3d); full VTE/cursor grid still deferred |
...
### Thermal pixel grid — ✅ DONE (#105, 2026-06-18)
Shipped exactly per the recommended on-demand design below: gateway `GET /api/thermal/frame` proxies the
SensorHead dashboard's `/api/thermal/data` (graceful 503 + empty frame on a non-sensor node) → ui-slint
polls it on an adaptive cadence (2s while a sensor answers, 30s otherwise) → builds a 32×24 **ironbow**
`Image` (`SharedPixelBuffer<Rgba8Pixel>` / `Image::from_rgba8`, auto-ranged, `image-rendering: pixelated`)
rendered in the SensorView THERMAL section. Live on apex1's MLX90640. Unit-tested (`ironbow`,
`build_thermal_image`). The original analysis is retained below for context.
...
### Council badge + read-only terminal — ✅ both shipped (G3d; original notes below)
