...
| **G3d** ⚙ | Council + Terminal apps | **Terminal DONE** (built, clean compile; live confirm pending): read-only-ish line-mode PTY pane over `/terminal-ws` — Rust owns the WS (lazy-spawned on first launch, reconnects w/ backoff), streams ANSI-stripped output into `terminal-text` (60 KB ring buffer) + bumps `terminal-scroll-tick`; `TextInput` submits a line → `terminal-send` → stdin. No cursor grid (curses apps garble — real VTE deferred). New: `components/terminal_view.slint`, `AppKind::terminal` (ord 5), start-menu entry. **Still TODO:** Council app (sub-agent session tree + taskbar badge — wire the `Council*` event set). |
