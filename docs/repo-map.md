...
| **apexos-confine** | `apexos-confine` (repo root) | Path-confinement primitives — the FS-sandbox *algorithm* (policy via `confine_fs`, IO via `openat2` `Beneath`). Policy *values* live in apexos-tools `tools.rs::confine`. | `src/lib.rs` + `src/openat.rs` | (none) |
