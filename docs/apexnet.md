...
| **P5c — The radio lane** ✅ | `/mesh-bridge` socket (the bridge dials agentd, sensor-bridge pattern), `BleGossipTransport`, `/api/connectivity` reports real lane health + the brainstem's view, `POST /api/mesh/gossip` | **Yes** | agentd → bridge → UART → brainstem outbox → sealed radio → peer, proven end to end; no bridge = honest 503, not an error |
| **P5d — The remaining lanes** | `WifiLan` (wrapping today's HTTP mesh paths) and `Courier` (the outbox) registered with the router; a2a routed by class rather than by tool | Sim + LAN | Kill Wi-Fi mid-session → a2a continues over BLE, artifact lands in outbox; restore → drains |
