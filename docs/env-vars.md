...
| `AGENTD_AGENT_ID` | `APEX` | agentd: the node's bound agent identity. agentd **stamps** it onto every Cerebro tool call (overriding the model) and uses it for its own Cerebro writes — single source of truth, see `docs/agent-identity.md`. Per-session binding is shipped — `hello{agent_id}` binds a session (slice 3e) |
...
| `AGENTD_MESH_WORKERS` | `1` | agentd: W2 mesh-worker kill switch. `0` refuses BOTH directions — `task_fanout{node}` on this node (conducting) and every `/api/worker/*` mesh endpoint (hosting). Boot-read; restart to change |
...
| `WORKER_MAX_STEPS` | `12` | agentd: step ceiling for `worker_report{continue}` loops — at the ceiling the worker goes Done ("step budget reached"). Clamped 1–100. Mandala cells override this with their own `budget.steps`, and R-cells renew past it while their measure falls (M1c) |
...
| `AGENTD_IDENTITIES` | `/etc/agentd/identities.toml` | agentd: the multi-agent identity registry (`[[user]]` + `[[agent]]`); see `docs/agent-identity.md`. Drives `/api/identities`, the login flow, and per-agent souls |
...
| `APEXOS_UI_SNAPSHOT_ADDR` | `127.0.0.1:8788` | ui-slint: loopback bind for the screen-mirror snapshot server (`take_snapshot`→PNG); the `screenshot_mirror` tool fetches from the matching `APEXOS_UI_SNAPSHOT_URL` (`http://127.0.0.1:8788/snapshot`); `APEXOS_UI_STATE_URL` (`http://127.0.0.1:8788/state`) is the `/state` twin `ui_query` fetches |
...
| `APEXOS_SONUS` | `0` | install.sh (boot-file/install.conf): `1` opts the node into Sonus-RS provisioning (clone+build sibling, self-loaded `/etc/sonus/env`, MCP plugin). CLI: `--sonus` / `--no-sonus`. See `docs/sonus.md` |
| `SONUS_AUDIO_DEVICE` | auto | gateway `/api/sonus/play`: ALSA device override for kiosk playback |
| `SUNO_DOWNLOAD_DIR` | `workspace/sonus` | sonus-mcp: where downloaded tracks land (`sonus_dir()`) |
