...
| `AGENTD_HISTORY_TOKEN_BUDGET` | `120000` | per-session in-memory history window (rough tokens, per-block-type calibrated ±20%). Soft ceiling with hysteresis: trim fires past 1.2×, cuts to 0.75×, oldest whole turns drop with an honest trim marker at the seam. `0` disables trimming. Lower it for small-context local models. **Seed-only since `#328`** — the Settings HISTORY WINDOW control / `POST /api/history` persists a choice that wins on restart |
...
| `MESH_BRIDGE_TOKEN` | *(empty)* | Bearer token for `/mesh-bridge`, sent as a header (not `?token=`, which lands in logs). Empty = no auth, matching the sensor-bridge convention. Its own token on purpose: a bridge that can inject mesh frames is a different trust grant from one that can inject sensor readings |
