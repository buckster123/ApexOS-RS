...
- **Status sweep (2026-07-09)**: re-verified every finding against current code. The waves were largely executed (commits `1494e0b`/`f50ba92`/`700c739`/`39a5fb1`/`384582b`, graph prune #95, vec0 upsert #141, cognitive_bootstrap `9c59b26`, audit-log wiring #243) — **21 of 30 FIXED**, 1 mitigated at the agentd layer (CB-017), 8 still open (CB-003/007/008/009/018/019/029 + CB-021, which is now WIDER than audited — all three unbounded tables grow autonomously since #243 wired the audit log live and the dream went nightly; retention remains the ask). Per-finding **Status** lines below; evidence text and line numbers remain the audit-time snapshot.
...
- **Status (2026-07-09)**: **still OPEN, and WIDER than audited** — no retention path exists for any of the three tables, and two narrowings no longer hold: (1) audit_log is now a LIVE write path (#243 wired `log_audit_event` at the dispatch chokepoint — every successful mutating tool call logs a row via the `audit_action` whitelist), and (2) `dream_run` IS auto-scheduled now (agentd's nightly `AGENTD_DREAM_CRON`, default 03:00 UTC), so dream_reports grows nightly, not operator-paced. All three tables grow autonomously; retention remains the ask.
