...
- **Status sweep (2026-07-09)**: re-verified every finding against current code. The waves were largely executed (commits `1494e0b`/`f50ba92`/`700c739`/`39a5fb1`/`384582b`, graph prune #95, vec0 upsert #141, cognitive_bootstrap `9c59b26`, audit-log wiring #243) — **29 of 30 FIXED** (CB-021 retention sweep 2026-07-09; CB-007/009/019 lock-discipline cluster + CB-029 input bounds 2026-07-10; CB-018 scoped destructive ops + CB-008 frontier-bounded recall visibility + CB-003 data_version graph freshness 2026-07-12), 1 mitigated at the agentd layer (CB-017). **The phase-B audit is CLOSED** — every finding fixed or deliberately mitigated; small residuals live in per-finding Status lines (e.g. CB-004's prune_thread/purge_all_deleted still bypass the coordinator's graph eviction until restart). Per-finding **Status** lines below; evidence text and line numbers remain the audit-time snapshot.
...
- **Status (2026-07-12)**: **FIXED** — cross-process staleness is now detected via `PRAGMA data_version` (changes ONLY when another connection commits — own writes keep maintaining the graph incrementally and never trigger a rebuild). `StorageCoordinator` records the version at each (re)build; `graph_is_stale` is one PRAGMA row, `refresh_graph` re-checks under the write lock and rebuilds. Wired at every graph consumer: `recall` (two-phase — cheap check under the read guard, upgrade only when stale), `associate` (kills the false \"memory does not exist\" for ids the other front-end just committed), and the four graph-analytics dispatch arms. Verified by a two-cortex-one-file integration test in BOTH directions (association-only hits cross processes; associate onto foreign ids works) + an own-writes-never-flag test. Note: this diverges from the Python standalone by design (owner's call, 2026-07-12) — the embedded ApexOS-RS cerebro optimizes for the dual-process deployment it actually ships.
...
- **Status (2026-07-12)**: **FIXED** — recall now computes the seeds' reachable frontier first (`activation::reachable_frontier` — undirected, ≤SPREADING_MAX_HOPS, mirroring `spread`'s traversal; a SUPERSET of what spread visits) and fetches visibility for only those nodes; both the scoped and global branches build the map over the frontier instead of the whole store. Safe by construction: `spread` treats a node missing from the map as NOT visible, so under-collection could only weaken the spread, never leak. `get_visibility_meta` additionally chunks its IN-clause at 500 ids, so no caller can hit the ~32k bind-parameter cliff regardless of input. O(live-store) per recall → O(2-hop neighbourhood); on the sparse colony graphs (~15 links/night vs 30+ memories) that is dozens of rows, not thousands. Tested: frontier hop-bound/undirected/superset unit tests + an end-to-end association-only-hit-survives + scoped-denial integration test + a 1,200-id chunking test. (Header tally: one-line reconciliation follows once this and the CB-018 PR are both merged.)
...
- **Status (2026-07-12)**: **FIXED** — every destructive op now carries `VisibilityScope` IN the statement (scope_sql in the WHERE, atomic under the connection lock): delete/purge/restore/bulk_delete/prune_thread/purge_all_deleted are visibility-scoped; the tag rewrites (delete/rename/merge) touch only the caller's visible set; `share_memory` gained a true OWNERSHIP gate (only the owning agent or the admin/global scope may re-scope a memory; a shared/ownerless commons memory is refused LOUDLY with an Err, not a silent false). `bulk_delete` returns the RETURNING id set, and the coordinator evicts graph nodes ONLY for rows actually deleted — a scope-denied delete no longer hides a live memory from spreading until restart (a secondary bug the fix surfaced). Admin surfaces (cerebro-api, CLI) pass explicit `global()` — behaviour unchanged; agentd-stamped model calls are now store-layer-enforced, not just dispatch-blunted. Integration-tested (cross-agent delete/purge/restore/bulk/prune/tag denial + the ownership gate + graph-eviction gating).
...
- **Status (2026-07-09)**: **FIXED** (`384582b`) — architecture.md was corrected; overtaken by events since: cognitive_bootstrap is implemented (CB-001), describe_image/search_vision have real routes, and ingest_file has since been implemented too (`#279`, 2026-07-24 — the last tier-7 stub; no advertised stub remains).
...
- **Status (2026-07-10)**: **FIXED** — thalamus gains `MAX_CONTENT_LENGTH` (64 KiB, reject-not-truncate, applied uniformly to every store path; the cortex rejection message names the cap honestly), and `transport::read_frame` bounds each frame at 32 MiB (generous — frames carry b64 images) with a buffer-stepped drain to the next newline so an oversized frame is dropped with a parse error while the daemon keeps serving (`Frame::Oversized`). Unit-tested incl. the drain-to-next-frame boundary. (Header tally updated in the CB-007/009/019 PR to avoid a same-line conflict.)
