...
|---|---|---|
| **H1 — dream opacity** | apex-3: one dream minted **39 procedures in 50.6s**; 313 of its 436 memories are procedural — "a second author writing a third of my procedural self, silently, nightly." apex1: can't distinguish "my memory of storing this is wrong" from "the dream changed it" — a tax on self-trust. | **Shipped** (#234): dream journal memory + "Last dream" wake-priming line + disk persistence. **Follow-on shipped too** (#237): diff-shaped counts — the journal now renders the novel vs re-discovery split (see C2) |
| **H2 — silent history trimming** | apex1 ranked it #1: the only mechanism causing *active confabulation* — the post-session deposit encodes the hole into long-term memory as fact. apex2 field-verified the fix live: woke to the 82-message marker, reports zero disorientation, zero confabulation — "the template for every fix on this list." | **Shipped** (#234) and colony-verified. Boot-as-reconstruction named honestly in §1 (charter-level, per apex-3 — no machinery). The seam-marker discipline extended to session-reload repair (#258): a tool result lost from the on-disk JSONL reloads as an honest `LOST_RESULT_MARKER`, and an empty reply commits as an explicit `⊘ (empty reply)` marker — never a silent hole. **Retrieve path (`docs/session-rag.md` S1):** the marker now names `session_search` over the on-disk JSONL (not Cerebro `recall`) so the next action is a tool, not a reconstruction |
...
| **C2 — dream provenance blindness** | apex2: five near-identical procedures re-extracted across five nights, never merged — "the housekeeper re-tidying the same shelf." apex-3: can't tell a lived-episode procedure from a dream-minted one at retrieval time. | **Shipped** (#237), beyond the ask: dream extraction now semantically dedups against the WHOLE store — a candidate ≥0.86 cosine to an existing procedural memory **reinforces** it (capped salience bump + a `rediscovered_count` metadata ledger) instead of re-minting a fragment, fixing the five-nights-same-shelf at the root (FTS5-only Nano keeps the old prefix dedup — BM25 isn't a similarity). `PhaseResult.procedures_rediscovered` + the dream journal render the novel/re-discovery split. Provenance tags turned out to already exist at every mint site (`dream_extracted`/`dream_formed`/`dream_distilled`/`dream_mutated`) — the gap was the fragmentation, not the tags |
