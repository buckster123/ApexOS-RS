...
- **EvolutionId derived from per-turn ActionId collides across turns** — `agentd/crates/plugins/src/supervisor.rs:303`; `main.rs:301,464,490,692`. Per-turn reset means evolutions repeatedly get EvolutionId(1); rollback_store overwrites undo snapshots; cold-start restore rebuilds corrupted map → rollback restores wrong soul.md/policy. Use a process-global AtomicU64. **[high — latent until self-evolution exercised]**
...
- **agentd rollback-store cold-start parses a text format the Rust `list_episodes` never emits** — `agentd/.../main.rs:654-668` vs `cerebro-mcp/.../dispatch.rs:604-610`. Parser expects Python Cerebro 0.5.1 `- ep_…` lines; Rust returns a JSON array. Cold-start rollback store rebuilds empty (silently). Parse the JSON array. **[high → medium: in-process rollback still works; only post-restart recovery breaks]**
- **Evolution undo-snapshot step dropped: agentd reads `memory_id`, store returns `id`** — `agentd/.../main.rs:607` vs `cerebro-mcp/.../dispatch.rs:158-164`. `parse_cerebro_id(out,"memory_id")` finds nothing → episode created with no recoverable undo step (cold-start path). Read `"id"`. **[high → medium: cold-start path only]**
...
- ◑ **Shared Event types — `apexos-protocol` crate** — ✅ slice 1 done: wire types extracted from `apexos-core` into a lean serde-only `apexos-protocol` crate (core re-exports it; agentd untouched, proven by green `--workspace` build+test). **Slice 2 open**: convert `ui-slint` to deserialize WS frames into the typed `Event` (kills the silent-drop string-matching at `main.rs:3461`). The old "blocked on publishing core" framing was stale — it's all one workspace. **[was low → slice 2 medium]**
