...
- **Per-agent ("agent-locked") workspaces** — *the convergence of the identity arc (#81) and the FS-confinement model (#68); spec'd build-ready.* The *"lock OS access behind approvals, free reign inside the workspace"* model **already exists** — it's `tools.rs::confine()` + the policy `workspace` rule (writes inside `AGENTD_WORKSPACE` = free; outside = ask/blocked). The new dimension is making the workspace **per-agent** instead of one global. **Gap:** `confine()`/`workspace_root()` read a *process-global* `AGENTD_WORKSPACE`, and apexos-tools is one process for all agents — FS tools never receive the caller's `agent_id`. **Slice:** (1) supervisor stamps the resolved agent's workspace onto FS-tool calls (mirror `stamp_agent_id` for `cerebro` — a system-set arg the model can't spoof); (2) `confine()` resolves per-call → `<base>/workspaces/<agent_id>/`, APEX/unbound → today's default `AGENTD_WORKSPACE` (**byte-identical**); (3) provision the dir on agent-create (next to `souls/<id>.md`). Keep the gateway's `resolve_workspace_path`/`_write_path` (HTTP images/audio) + `policy.rs::workspace_decision` in agreement — all three confinement points must resolve the same per-agent root. Security-sensitive (it IS the confinement boundary) → land as its own slice with the `confine` tests extended per-agent. **[medium — high value, do deliberately]**
