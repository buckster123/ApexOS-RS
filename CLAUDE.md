...
- **Identity/memory**: `agent_id` is system-**stamped**, never model-supplied · per-agent workspace stamping · CCBS boot priming · evolution undo snapshots private + H4 snapshot gate (owner+target, consume after commit) · PAC lint gate · `soul_rehearse` · wakeup bounds
