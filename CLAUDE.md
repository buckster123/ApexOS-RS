...
- **Smoker mode — runtime sensor-alert sensitivity for a smoker's baseline.** A cigarette / lighter near the sensor head reads as **IAQ ~170–250** (the BME688 air sensor is the sensitive one — even a secondhand whiff trips it) and a **50–80 °C** thermal blip for up to ~5 min, which crosses the normal thresholds (IAQ 150 / thermal 45 °C) → an autonomous alert on every cig. **Smoker mode** raises the IAQ + thermal thresholds *above* the cig range (defaults IAQ 250 / thermal 85 °C, env `SENSOR_SMOKER_IAQ_THRESHOLD`/`SENSOR_SMOKER_THERMAL_THRESHOLD`) and lengthens the persistence window beyond a cigarette (default 120s, `SENSOR_SMOKER_PERSIST_SECS`) — so smoking stays quiet while a *sustained, hotter* real event (a developing fire) still alerts (unit-tested: cig-range → Clear under smoker thresholds, IAQ 320 / 120 °C → still Candidate). It's a **runtime toggle**, not a restart: the alert loop reads a shared `AtomicBool` per reading; `GET`/`POST /api/sensors/config {smoker_mode:bool}` flips + persists it to `<log_dir>/sensor_config.json` (agentd seeds the flag from that file at boot via `sensor_config::load_smoker_mode`; the gateway owns the write — one toggle path). The classifier/persistence stay unchanged — smoker mode only swaps which thresholds feed them. *(A Settings/Sensor UI toggle rides on this same API — follow-up.)*
