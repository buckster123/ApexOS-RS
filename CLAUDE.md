...
- **FS/safety**: confinement lives in the tool (`apexos-confine`) · FS IO via `openat2` `RESOLVE_BENEATH|NO_SYMLINKS` · git roots · USB exo-workspace under the workspace · `media/` root-owned, helpers lstat+mountinfo · eject via root systemd unit, never sudo (`NoNewPrivileges`)
