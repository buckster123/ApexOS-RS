...
- **ApexNET radio**: brainstem commissioning (asymmetric acceptance, TOFU on the wire) · counters RESERVED not recorded (a repeat is a key compromise) · the `apexnet` partition split · gossip advertises NON-connectable (a connection silences a node permanently) · scan with duplicate filtering OFF · seal-at-drain for the outbox · replay windows never forget · router class policy (Critical fans out ⇒ the seen-cache is mandatory) · `/mesh-bridge` dials IN and needs its own token · `POST /api/mesh/gossip` is admin-only (unicast, bounded, not a mesh-token route)
