...
- **Shader/3D face — embodiment Phase 2** — Phase 1 (emote *control*) shipped (#51): APEX drives its Slint face via the `display_face` tool — 12 expressions + gaze + intensity, held past turn-end (see the face-two-layers gotcha). Phase 2 (GL *render*) **default on GL tiers** (#52 spike → 4 slices): a face renders in-window via `Window::set_rendering_notifier` + femtovg `GraphicsAPI::NativeOpenGL` + `glow` — no Skia, no 2nd window/process; auto-enabled wherever a real GL context exists, 2D fallback otherwise (Nano), `APEX_FACE_GL=0` to force 2D. The arc: **(1)** `glScissor` the draw to the Face-window rect (rect published via the `FaceGl` global, sampled by a Timer); **(2)** emote uniforms (accent/eyes/brows/mouth/gaze, mirrored from `FaceView` so the GL + 2D faces can't drift); **(3)** raymarched-SDF head — lit ellipsoid + protruding nose, ink features on the true 3D normal, head-turn on gaze; **(4)** promoted to default (auto-detect) + redraw loop gated on a visible Face window so it's free on the kiosk when closed. Verified on desktop winit via the snapshot server (`take_snapshot()` captures the GL overlay). **Remaining: validate on the Pi's V3D** (only on-hardware gate left).
