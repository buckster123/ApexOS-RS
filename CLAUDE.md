...
- **Build/deploy**: fontconfig + KMS build deps · `text file busy` · low-RAM OOM build guard · verify deploys by *running commit* (`health.json`) · `apexos-update` idempotency (`install.conf`) · policy additive sync (quote-insensitive, validate-before-persist) · self-update watchdog never trusts a `staged` path from JSON · `apply_daemon_update` has no `test_cmd` (review then locked isolated build) · kiosk `ui.env` (token-only; `User=apexos-ui`)
...
- **agentd core**: per-session turn gate · history trim + honest markers · `session_search` over JSONL (not Cerebro) · session JSONL append order + `repair_history` · delete/archive tombstone blocks late persist · worker fanout persists txn before ack · serde `#[derive(Default)]` shadowing trap · prompt-cache byte-stable prefix (never put volatile text in soul/embodiment/priming) · pending approvals are session+nonce bound and purged on cancel/complete/timeout · LAN login closed until owner PIN; guest session ≠ `/api/run` · policy mode persists to `policy.toml` before `{ok:true}`
- **Identity/memory**: `agent_id` is system-**stamped**, never model-supplied · caller aliases (`set_agent_id`/`from_agent_id`) stamped/clamped too · per-agent workspace stamping · CCBS boot priming · evolution undo snapshots private + H4 snapshot gate (owner+target, consume after commit) · PAC lint gate · `soul_rehearse` · wakeup bounds · torn `identities.toml` is quarantined, never treated as empty
- **ApexNET radio**: brainstem commissioning (asymmetric acceptance, TOFU on the wire) · counters RESERVED not recorded (a repeat is a key compromise) · flash-read of the high-water fails closed (never treat as 0) · sealed provision increments a persisted ctr · the `apexnet` partition split · gossip advertises NON-connectable (a connection silences a node permanently) · scan with duplicate filtering OFF · seal-at-drain for the outbox · replay windows never forget (persisted `ReplayTable`, never evict) · router class policy (Critical fans out ⇒ the seen-cache is mandatory) · `/mesh-bridge` dials IN and needs its own token · `POST /api/mesh/gossip` is admin-only (unicast, bounded, not a mesh-token route) · P5d: a2a is class-routed (WifiLan → BLE → courier); `send_to_agent` floor is `minimal`; bridge unit idles without `MESH_BRIDGE_DEV` (never guess a port)
- **Mesh/colony**: LAN bind + per-peer tokens · pairing never exchanges `AGENTD_TOKEN` · `x-mesh-hops` increments per hop · a2a lands in per-peer sessions · federation relays (provenance-stamped copies, import lands private, `shared_only()` wire boundary, dream-digest echo-guard) · beacon · capabilities · cross-node spawn guards · vast.ai bridge invariants · imaginarium sibling node (key isolation, INSTALLED≠ACTIVE, proxy-mode-only plugin) · sonus sibling (no-daemon key isolation, self-loaded env file, python-stanza cutover) · Tier-4 courier lane (sticks untrusted, AAD-bound seals, tamper fails closed+loud, PSK never on radio)
...
- **FS/safety**: confinement lives in the tool (`apexos-confine`) · FS IO via `openat2` `RESOLVE_BENEATH|NO_SYMLINKS` · tools units must not `RestrictNamespaces`/`ProcSubset=pid` (systemd 257 ENOSYS on `openat2`) · tools worker Landlock (same-uid `/var/lib/agentd` closed) · fs/shell empty netns, no `/dev` · net tools in `apexos-net` · camera/gpio in `apexos-dev` · git roots · git spawn env_clear + no hooks/aliases · USB exo-workspace under the workspace · `media/` root-owned, helpers lstat+mountinfo · eject via root systemd unit, never sudo (`NoNewPrivileges`) · MCP children `env_clear` (no `AGENTD_TOKEN` / provider keys in apexos-tools)
...
| `docs/session-rag.md` | Verbatim transcript retrieve — `session_search` / JSONL, not Cerebro |
