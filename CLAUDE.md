...
- **Shader/3D face — embodiment Phase 2** — Phase 1 (emote *control*) shipped (#51): APEX drives its Slint face via the `display_face` tool — 12 expressions + gaze + intensity, held past turn-end (see the face-two-layers gotcha). Phase 2 (GL *render*) pipeline **proved on Pi 5** (#52, env-gated `APEX_FACE_GL=1` spike): a GLSL face renders in-window via `Window::set_rendering_notifier` + femtovg `GraphicsAPI::NativeOpenGL` + `glow` — no Skia, no 2nd window/process; GL tiers only (Pi 4/5 V3D + x86; Nano keeps the 2D face). Next: confine the GL draw to the Face-window rect (`glScissor`, WM geometry) → drive uniforms from the emote state (state/gaze/intensity) → raymarched SDF face (lit head, eye sockets, brow ridges, emote morphs).
